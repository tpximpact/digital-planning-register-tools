// benchmark.ts - run with `bun workspace @dpr/converter-bops bun run src/benchmark.ts`
import { convertBopsApplicationToOdp } from '@dpr/converter-bops'
...
for (let i = 0; i < ITERATIONS; i++) {
  convertBopsApplicationToOdp(mockBopsInput)
}
const end = performance.now()

console.log(`${ITERATIONS} iterations in ${(end - start).toFixed(2)}ms`)
console.log(`${((end - start) / ITERATIONS).toFixed(4)}ms per call`)
