...
const BopsFileSchemaChecker = TypeCompiler.Compile(BopsFileSchema)

describe('BopsFile TypeBox schema', () => {
  const valid = validBopsFile
  it('validates a correct object', () => {
    expect(BopsFileSchemaChecker.Check(valid)).toBe(true)
  })

  it('rejects an invalid object', () => {
    const invalid = { ...valid, createdAt: 'large' } // createdAt should be a date
    expect(BopsFileSchemaChecker.Check(invalid)).toBe(false)
  })
  ...
  describe('uri field', () => {
    ...
    it('rejects an invalid uri field', () => {
      expect(
        BopsFileSchemaChecker.Check({ ...valid, url: 'invalid-uri' })
      ).toBe(false)
    })
  })
})
